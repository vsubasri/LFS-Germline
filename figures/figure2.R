...
allmutsTable <- read.csv('~/research/LFS Mut/maftools_input/lfs.wgsmeth.hg19.tp53.maf.txt',sep='\t',stringsAsFactors = F)
...
p53_maf <- read.maf(maf=allmutsTable,
                    cnTable=p53_cnTable,
                    clinicalData = '~/research/LFS Mut/maftools_input/lfs.clinical.wgsmeth.txt',
                    vc_nonSyn = vc.nonSilent,
                    useAll = TRUE)
p53_maf@variant.classification.summary <- str_replace_all(p53_maf@data$Variant_Type,"CNV","Deletion")
lollipopPlot(maf = p53_maf, gene = 'TP53', showMutationRate = FALSE,refSeqID = 'NM_000546',AACol='aaChange',labelPos=c('282','175','273','220','245','248'))
plotmafSummary(maf = p53_maf,  addStat = 'median',titvRaw = FALSE)
...
get_vcColors = function(alpha = 1, websafe = FALSE, named = TRUE){
  ...
  if(named){
    names(col) = names = c('Nonstop_Mutation','Frame_Shift_Del','ITD','Missense_Mutation','Silent','Nonsense_Mutation',
                           'RNA','Splice_Site','Intron','Frame_Shift_Ins','In_Frame_Del','Frame_Shift_Del_Ins','In_Frame_Ins',
                           'Translation_Start_Site',"Multi_Hit", 'Amp', 'Del', 'Complex_Event', 'pathway')
  }
...
lollipopPlot = function(maf, gene = NULL, AACol = NULL, labelPos = NULL, labPosSize = 0.9, showMutationRate = TRUE,
  ...
  if(cBioPortal){
    vc = c("Nonstop_Mutation", "Frame_Shift_Del", "Missense_Mutation","Frame_Shift_Del_Ins",
